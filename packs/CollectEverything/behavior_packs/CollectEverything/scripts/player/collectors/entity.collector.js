import { Player, World, EntityComponentTypes, } from "@minecraft/server";
...
export class EntityCollector {
    ...
    constructor(di, collect) {
        ...
        this.world.afterEvents.playerInteractWithEntity.subscribe(this.interactCallback);
        this.world.afterEvents.entityDie.subscribe(this.dieCallback);
        this.unsubscribeInteract = () => this.world.afterEvents.playerInteractWithEntity.unsubscribe(this.interactCallback);
        this.unsubscribeDie = () => this.world.afterEvents.entityDie.unsubscribe(this.dieCallback);
    }
    getEntityId(entity) {
        ...
        if (markVariantComponent) {
            const markValue = markVariantComponent.value;
    ...
    onPlayerInteractWithEntity(event) {
        ...
        const itemConsumed = beforeItem?.typeId !== afterItem?.typeId ||
            beforeItem?.amount !== afterItem?.amount;
        const itemUsed = beforeItem?.typeId === afterItem?.typeId &&
        ...
        this.logger.debug(`Interacted with entity but no significant action: ${entity.typeId}`);
    }
    onEntityDie(event) {
        ...
        if (!damagingEntity || damagingEntity.id !== this.player.id)
            return;
